...
func SaveRegisterAttendance(c echo.Context) error {
	...
	err = mysql.DB.Updates(&validateAttendance).Error
	if err != nil {
		return echo.NewHTTPError(http.StatusBadRequest, err.Error())
	}
	return c.JSON(http.StatusOK, map[string]string{
...
func GetAllAttendance(c echo.Context) error {
	...
	mysql.DB.Table("attendances a").Select("e.first_name, e.last_name, a.* ").Joins("INNER JOIN employes e on e.pin_employe = a.pin_employe_fk").Find(&attendance)
...
func ValidateHorary(c echo.Context) error {
	...
	mysql.DB.Raw("select arrival from attendances a where pin_employe_fk = ? and date_format(arrival, '%d-%m-%Y') = date_format(?, '%d-%m-%Y')",
